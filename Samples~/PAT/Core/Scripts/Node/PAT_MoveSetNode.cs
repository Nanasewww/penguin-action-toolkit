...
namespace PAT
{
    public class PAT_MoveSetNode : PAT_Node
    {
        //public List<PAT_ActionNode> children = new List<PAT_ActionNode>();
        //public MonoScript movesetScript;
        public void Initialize()
        {
            // Ensure attachedScripts list is initialized
            if (attachedScripts == null)
            {
                attachedScripts = new List<MonoScript>();
            }

            // Add the provided script to the list if it's not already added
            MonoScript script = AssetDatabase.LoadAssetAtPath<MonoScript>("Assets/PenguinActionToolkit/Core/Scripts/Character/MoveSet.cs");
            if (!attachedScripts.Contains(script))
            {
                attachedScripts.Add(script);
                //Debug.Log($"Script {script.name} added to {title}'s attached scripts.");
            }
        } 
    }

    #if UNITY_EDITOR
    [CustomEditor(typeof(PAT_MoveSetNode))]
    public class PAT_MoveSetNodeInspector : Editor
    {
        // Start is called before the first frame update
        private PAT_MoveSetNode node;
